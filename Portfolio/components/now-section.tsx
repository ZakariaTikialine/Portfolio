...
const lastUpdatedFallback: Record<Locale, string> = {
...
function NowSection() {
  ...
  const { data } = useSWR("/api/build-info", fetcher, {
  ...
  const lastUpdatedText = useMemo(() => {
    if (data?.lastUpdated) {
      ...
      return `${lastUpdatedLabel[locale]}: ${formatted}`
    }
    return lastUpdatedFallback[locale]
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="mx-auto max-w-xl rounded-xl border border-accent/20 bg-linear-to-br from-accent/5 to-secondary/5 p-4 sm:p-6 shadow-md backdrop-blur-md"
    >
      <div className="space-y-3 sm:space-y-4">
        {/* Header */}
        <div className="flex items-center gap-2">
          <div className="h-2 w-2 rounded-full bg-accent animate-pulse" />
          <h3 className="font-mono text-xs sm:text-sm font-semibold text-accent uppercase tracking-wide">
            now/
          </h3>
        </div>

        {/* Content */}
        <div className="space-y-1.5 sm:space-y-2 font-mono text-xs sm:text-sm text-muted-foreground">
          {entries.map((entry) => (
            <p key={entry.label}>
              <span className="font-semibold text-foreground">{entry.label}:</span> {entry.value}
            </p>
          ))}
          <p className="text-xs text-accent/70">{lastUpdatedText}</p>
        </div>
      </div>
    </motion.div>
  )
